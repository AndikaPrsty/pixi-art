...
type FrameData = { filename?: string };
...
export class SpriteSheetLoader {
  ...
  static isExtendedFormat(
    ...
    const sheet = spriteSheet as Partial<ExtendedSpriteSheet>;
    return Boolean(
      sheet.meta &&
  ...
  static convertExtendedToLegacy(
    ...
    console.log(
      `[SpriteSheetLoader] Converting extended format. FrameTags:`,
      extendedSheet.meta.frameTags,
    );

    if (
      extendedSheet.meta.frameTags &&
      extendedSheet.meta.frameTags.length > 0
    ) {
      ...
      for (const frameTag of extendedSheet.meta.frameTags) {
        ...
        console.log(
          `[SpriteSheetLoader] Processing frameTag: ${frameTag.name}, range: ${frameTag.from}-${frameTag.to}`,
        );

        // Handle both array and object frame formats
        let frameData: FrameData[];
        if (Array.isArray(extendedSheet.frames)) {
          frameData = extendedSheet.frames as FrameData[];
        } else {
          frameData = Object.values(extendedSheet.frames) as FrameData[];
        }

        console.log(
          `[SpriteSheetLoader] Total frames available: ${frameData.length}`,
        );
        ...
        for (let i = frameTag.from; i <= frameTag.to; i++) {
          if (i < frameData.length) {
            ...
            console.log(
              `[SpriteSheetLoader] Processing frame index ${i}, filename: ${frame.filename}`,
            );

            try {
              const texture = Assets.get(extendedSheet.meta.image);
              console.log(
                `[SpriteSheetLoader] Texture asset for ${extendedSheet.meta.image}:`,
                !!texture,
              );
              ...
              if (texture && texture.textures) {
                ...
                for (const key of possibleKeys) {
                  if (key && texture.textures[key]) {
                    ...
                    console.log(
                      `[SpriteSheetLoader] Found frame ${i} with key: ${key}`,
                    );
                    break;
                  }
                }
                ...
                if (frameTexture) {
                  ...
                  console.warn(
                    `[SpriteSheetLoader] No texture found for frame ${i}, tried keys:`,
                    possibleKeys,
                  );
                }
              }
              ...
              console.error(
                `[SpriteSheetLoader] Error getting texture for frame ${i}:`,
                error,
              );
            }
          }
        ...
        if (animationFrames.length > 0) {
          ...
          console.log(
            `[SpriteSheetLoader] Created animation '${frameTag.name}' with ${animationFrames.length} frames`,
          );
        } else {
          console.warn(
            `[SpriteSheetLoader] No frames found for animation: ${frameTag.name}`,
          );
        }
      }
        ...
        try {
          ...
          if (texture && texture.textures) {
            ...
            if (allFrames.length > 0) {
              ...
              console.log(
                `[SpriteSheetLoader] Created combined animation '${imageName}' with ${allFrames.length} frames`,
              );
            }
          }
        } catch (error) {
          console.error(
            `[SpriteSheetLoader] Error creating combined animation:`,
            error,
          );
        }
      }
      ...
      console.log(
        `[SpriteSheetLoader] No frameTags found, creating animation from all frames`,
      );
      ...
      if (Array.isArray(extendedSheet.frames)) {
        ...
        frameData = extendedSheet.frames as FrameData[];
        console.log(
          `[SpriteSheetLoader] Array format detected with ${frameData.length} frames`,
        );
      } else {
        // Object format: convert to array
        frameData = Object.values(extendedSheet.frames) as FrameData[];
        console.log(
          `[SpriteSheetLoader] Object format detected with ${frameData.length} frames`,
        );
      }
      ...
      if (frameData.length > 0) {
        try {
          ...
          console.log(
            `[SpriteSheetLoader] Available texture keys:`,
            texture ? Object.keys(texture.textures || {}) : "none",
          );
          ...
          if (texture && texture.textures) {
            ...
            if (animationFrames.length > 0) {
              ...
              console.log(
                `[SpriteSheetLoader] Created animation '${imageName}' with ${animationFrames.length}/${frameData.length} frames`,
              );
            } else {
              console.error(
                `[SpriteSheetLoader] No valid frames found for animation`,
              );
            }
          }
        } catch (error) {
    ...
    console.log(
      `[SpriteSheetLoader] Final animations:`,
  ...
  static loadSpriteSheet(name: string): LegacySpriteSheet | null {
    try {
      ...
      console.log(
        `[SpriteSheetLoader] Raw sprite sheet for ${name}:`,
        spriteSheet,
      );
      ...
      if (this.isExtendedFormat(spriteSheet)) {
        ...
        console.log(
          `[SpriteSheetLoader] Converted animations for ${name}:`,
          Object.keys(converted.animations),
        );
        return converted;
      }
      ...
      if (spriteSheet.frames && Object.keys(spriteSheet.frames).length > 0) {
        console.log(
          `[SpriteSheetLoader] Loading individual frames format: ${name}`,
        );
      ...
      console.warn(
        `[SpriteSheetLoader] Unknown sprite sheet format for: ${name}`,
      );
      return null;
    } catch (error) {
      console.error(
  ...
  static convertFramesToLegacy(
    ...
    console.log(
      `[SpriteSheetLoader] Converting individual frames format for ${name}`,
    );
    ...
    if (frameKeys.length > 0) {
      try {
        ...
        console.log(
          `[SpriteSheetLoader] Texture asset for ${framesSheet.meta.image}:`,
          !!texture,
        );

        if (texture && texture.textures) {
          const animationFrames = frameKeys
            .map((frameKey) => texture.textures[frameKey])
          ...
          if (animationFrames.length > 0) {
            ...
            console.log(
              `[SpriteSheetLoader] Created animation '${name}' with ${animationFrames.length} frames`,
            );
          }
        }
      } catch (error) {
        console.error(
          `[SpriteSheetLoader] Error creating animation from frames:`,
          error,
        );
      }
    }
