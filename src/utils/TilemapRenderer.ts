...
export class TilemapRenderer {
  ...
  public async loadTilesets(): Promise<boolean> {
    ...
    for (const tileset of this.tilemapData.tilesets) {
      try {
        ...
        if (texture) {
          ...
          console.warn(
            `[TilemapRenderer] Could not load tileset: ${tileset.name}, image: ${imagePath}`,
          );
        }
      } catch (error) {
        console.warn(
          `[TilemapRenderer] Failed to load tileset ${tileset.name}:`,
          error,
        );
      }
    }
    ...
    console.log(
      `[TilemapRenderer] Loaded ${loadedCount}/${this.tilemapData.tilesets.length} tilesets`,
    );
    console.log(
      `[TilemapRenderer] Generated ${this.tileTextures.size} tile textures`,
    );
  ...
  public renderLayers(container: Container): Container[] {
    ...
    console.log(
      `[TilemapRenderer] Rendering ${this.tilemapData.layers.length} layers`,
    );
    ...
    for (const layer of this.tilemapData.layers) {
      ...
      if (layerContainer.children.length > 0) {
        ...
        console.log(
          `[TilemapRenderer] Rendered layer: ${layer.name} with ${layerContainer.children.length} tiles`,
        );
      }
    }
  ...
  private renderTileLayer(layer: TileLayer, container: Container): void {
    ...
    if (Array.isArray(layer.data)) {
      ...
      console.warn(
        `[TilemapRenderer] Unknown tile data format for layer ${layer.name}`,
      );
      return;
    }

    console.log(
      `[TilemapRenderer] Rendering ${layer.name} with ${tileData.length} tiles, tilesize: ${tilewidth}x${tileheight}`,
    );
    ...
    for (let y = 0; y < layer.height; y++) {
      for (let x = 0; x < layer.width; x++) {
        ...
        if (!tileTexture) {
          ...
          if (tilesRendered === 0) {
            console.log(
              `[TilemapRenderer] Missing texture for gid ${gid} in layer ${layer.name}`,
            );
          }
          continue;
        }
    ...
    console.log(
      `[TilemapRenderer] Layer ${layer.name}: rendered ${tilesRendered} tiles out of ${tileData.length} total`,
    );
  }
  ...
  private decodeBase64TileData(base64Data: string): number[] {
    try {
      ...
      console.log(
        `[TilemapRenderer] Decoded ${tileData.length} tiles from base64`,
      );
      return tileData;
    } catch (error) {
      console.error(
        "[TilemapRenderer] Failed to decode base64 tile data:",
        error,
      );
      return [];
    }
