...
export class Map {
  ...
  private async addTiledObjects(): Promise<void> {
    try {
      ...
      for (const [index, layer] of parkMapData.layers.entries()) {
        console.log(
          `[Map] Layer: ${layer.name}, type: ${layer.type}, visible: ${layer.visible}`,
        );
  ...
  private async processObjectLayer(
    layer: TiledLayer,
    index: number,
  ): Promise<void> {
    console.log(`[Map] Processing object layer: ${layer.name}`);
    console.log(
      `[Map] Layer visible: ${layer.visible}, objects: ${layer.objects?.length || 0}`,
    );
    ...
    for (const object of layer.objects) {
      ...
      console.log(
        `[Map] Processing object: ${object.name} at (${object.x}, ${object.y})`,
      );
      await this.createObjectSprite(object, index);
    }
  }

  private async createObjectCollider(object: TiledObject) {
    ...
    // Add collider to array for collision checking
    this.colliders.push(collider);

    // Visual debug representation (optional - can be disabled in production)
    const box = new Graphics()
      .rect(object.x, object.y, object.width ?? 0, object.height ?? 0)
      .fill({ color: 0xff0000, alpha: 0.1 })
      .stroke({ color: 0xff0000, width: 1, alpha: 0.3 });
    this.objectContainer.addChild(box);
  }
  ...
  private async createObjectSprite(
    ...
    try {
      ...
      if (spriteSheet && spriteSheet.animations) {
        ...
        await this.createAnimatedObject(object, spriteSheet, index);
      } else if (object.name === "collider") {
        this.createObjectCollider(object);
      } else {
        // Try to create static sprite if no animation
        const texture =
          Assets.get(`${object.name}.png`) ||
          Assets.get(`${object.name}-0.png`);
        if (texture) {
  ...
  private async createAnimatedObject(
    ...
    const animationFrames = spriteSheet.animations[animationKey];
    if (!animationFrames) {
      console.warn(`[Map] No animation frames found for ${object.name}`);
      return;
    }
    ...
    const box = new Graphics()
      ...
      .stroke({ color: 0x00ff00, width: 1, alpha: 0.3 });
    ...
    console.log(
      `[Map] Added animated object ${object.name} at (${animatedSprite.x}, ${animatedSprite.y})`,
      animatedSprite,
    );
  }

  private createStaticObject(object: TiledObject, texture: PixiTexture): void {
    ...
    console.log(
      `[Map] Added static object ${object.name} at (${sprite.x}, ${sprite.y})`,
    );
  }

  private setupObjectInteraction(
  ...
  public async addPlayer(player: AnimatedSprite) {
    console.log(
      "[Map] Adding player to object container at position:",
      player.x,
      player.y,
    );
    console.log("[Map] Player dimensions:", player.width, player.height);
    console.log(
      "[Map] Object container children before adding player:",
    ...
    console.log(
      "[Map] Object container children after adding player:",
      this.objectContainer.children.length,
    );
  ...
  public updateDepthSort(): void {
    ...
    console.log(
      "[Map] Object children before sort:",
      this.objectContainer.children.length,
    );
    ...
    console.log(
      "[Map] Object children after sort:",
      this.objectContainer.children.length,
    );
    ...
    this.objectContainer.children.forEach((child, index) => {
      ...
      console.log(
        `[Map] ${index}: bottom=${bottom}, x=${child.x}, y=${child.y}`,
      );
      child.zIndex = index;
    });
  }
  ...
  public checkCollision(rect: Rectangle): boolean {
    return this.colliders.some((collider) => {
      return (
        rect.x < collider.x + collider.width &&
